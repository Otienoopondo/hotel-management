...
export default function ClientSignup() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      await axios.post('/client/signup', formData);
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <main className="flex flex-col lg:flex-row flex-grow">
        <div className="lg:w-1/2 p-8 flex flex-col justify-center items-start space-y-6">
          <h2 className="text-3xl font-bold text-yellow-400">Create Client Account</h2>
          ...
          <p className="text-sm mt-4 text-gray-300">
            ...
            <button onClick={() => router.push('/client/login')} className="text-yellow-400 underline hover:text-yellow-300">
              Log In
            </button>
          </p>
        ...
        <div className="hidden lg:block lg:w-1/2 relative h-96 lg:h-auto">
          <Image src="/csignup.jpg" alt="Client Background" layout="fill" objectFit="cover" priority />
        </div>
      </main>
      ...
      {showAbout && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-gray-800 text-white p-6 rounded-lg max-w-md">
            ...
            <p>Welcome to our hotel management system for Clients, ensuring seamless reservations and experiences.</p>
